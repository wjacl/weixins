	...
	function WXImgUploader(max,min,uploadInput,countDomId,uploaderFilesDomId){
	    ...
	    this.upload = function(){
	    	...
	    	if(this.localIds.length > this.uploadOk.length){
    			...
    			this.needUploadImgs = this.localIds.length;
    			
    			for(var i = 0; i < this.localIds.length; i++){
    				wx.uploadImage({
        	    		localId: uploadObj.localIds[i], // 需要上传的图片的本地ID，由chooseImage接口获得
        	    		isShowProgressTips: 0, // 默认为1，显示进度提示
        	    		success: function (res) {
        	    			uploadObj.uploadedServerIds.push(res.serverId);
        	    			$("img[src='" + uploadObj.localIds[i] + "']").attr("data-id",res.serverId);
        	    			uploadObj.uploadOk.push({"localId" : uploadObj.localIds[i],"serverId" : res.serverId});
        	    		}
        	    	});
    			}
    			...
    			wx.uploadImage({
    	    		...
    	    		success: function (res) {
    	    			...
    	    			uploadObj.uploadOk.push({"localId" : lid,"serverId" : res.serverId});
    	    		}
    	    	});
    			}); */
	    	}
		            	...
		            	if(local == "local"){
		            		...
		            		for(var i = uploadObj.uploadOk.length - 1; i > 0 ; i--){
		            			if(uploadObj.uploadOk[i].localId == src){
		            				for(var j = uploadObj.uploadedServerIds.length - 1; j > 0; j--){
		            					if(uploadObj.uploadedServerIds[j] == uploadObj.uploadOk[i].serverId){
				            				uploadObj.uploadedServerIds.splice(j, 1);
				            				break;
				            			}
		            				}
			...
			if(imup1.upload()){
				...
				function ccck(){
					if(imup1.uploadOk.length < imup1.localIds.length && xx < 10){
						...
						alert(JSON.stringify(imup1.localIds));
						alert(JSON.stringify(imup1.uploadedServerIds));
						alert(JSON.stringify(imup1.uploadOk));
						weui.loading().hide();
